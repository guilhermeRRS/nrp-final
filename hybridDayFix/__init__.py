...
class Hybrid:
    ...
    def run(self, startObj, runRandom:bool = True, noRelax = False):
        ...
        while self.chronos.stillValidMIP() and keepFix:
            ...
            if runRandom:
                for i in range(10):
                    ...
                    self.run_internal_dayDayInnerFix(time, numberOfDays, numberOfNurses)
            else:
                for posI in range(0, self.nurseModel.I, numberOfNurses):
                    if not self.chronos.stillValidMIP():
                        break
                    for posD in range(0,self.nurseModel.D, numberOfDays):
                        ...
                        if noRelax:
                            self.nurseModel.model.m.setParam("NoRelHeurTime", time)
                        self.run_internal_dayDayInnerFix(self.chronos.timeLeftForVND(), numberOfDays, numberOfNurses, False, markD, posI)
