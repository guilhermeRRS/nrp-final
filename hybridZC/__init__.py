...
class Hybrid:
    ...
    def run(self, startObj, numberOfNurses_fixDay = 5, numberOfNurses_fixNurse = 2, numberOfDays = 12):
        ...
        while self.chronos.stillValid() and keepFix:
            ...
            if True:
                if runRandom:
                    for i in range(max(1, math.ceil(self.nurseModel.I/numberOfNurses_fixNurse))):
                        if not self.chronos.stillValid():
                            break
                        print("--> ",i)
                        time = max(min(120, self.chronos.timeLeft()), 1)
                    ...
                    for i in range(max(1, math.ceil(self.nurseModel.I/numberOfNurses_fixDay)*math.ceil(self.nurseModel.D/numberOfDays))):
                        if not self.chronos.stillValid():
                            break
                        print("--> ",i)
                        time = max(min(120, self.chronos.timeLeft()), 1)
