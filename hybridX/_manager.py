...
def run_internal_innerFix(self, time, numberOfNurses: int, runRandom: bool = True, pos:int = 0):
    ...
    if gurobiReturn.valid():
        ...
        if newObj < self.penalties.best: #here changes both
            #input("@")
            self.penalties.best = newObj
            self.chronos.printMessage("nFixs+", newObj)
            for i in range(self.nurseModel.I):
                for d in range(self.nurseModel.D):
                    for t in range(self.nurseModel.T):
...
def run_internal_dayDayInnerFix(self, time, numberOfDays:int, numberOfNurses: int, runRandom: bool = True, posDay:int = 0, posNurse:int = 0):

    numberOfDays = min(self.nurseModel.D, numberOfDays)
    ...
    if runRandom:
        ...
        dayStart = random.randint(0, self.nurseModel.D-numberOfDays)
        dayEnd = dayStart+numberOfDays
        ...
        dayStart = posDay
        dayEnd = min(dayStart + numberOfDays, self.nurseModel.D)
    ...
    self.nurseModel.model.m.update()
    self.chronos.startCounter("START_OPTIMIZE_INNERd")
    self.nurseModel.model.m.optimize()
    self.chronos.stopCounter()
    
    gurobiReturn = GurobiOptimizedOutput(self.nurseModel.model.m)

    self.chronos.printObj("ORIGIN_SOLVER", "SOLVER_GUROBI_OUTPUTd", gurobiReturn)
    ...
    if gurobiReturn.valid():
        ...
        if newObj <= self.penalties.best: #here changes both
            #input("@")
            self.penalties.best = newObj
            self.chronos.printMessage("dFixs+", newObj)
            for index in range(len(nursesFree)):
                i = nursesFree[index]
                for d in range(dayStart, dayEnd):
                    for t in range(self.nurseModel.T):
...
def run_internal_all(self, time, final = True):
    ...
    if final:
        ...
        self.chronos.startCounter("OPTIMIZE_ALL_INNER")
    self.nurseModel.model.m.optimize()
    self.chronos.stopCounter()
