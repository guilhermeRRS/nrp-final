...
def run_internal_shiftFix(self, time, mode:string):
    ...
    if mode == "workFree":
        ...
        for i in range(self.nurseModel.I):
            for d in range(self.nurseModel.D):
                allowFree = False
                if sum(self.currentSol.solution[i][d]) == 1:
                    if d < 2 or d > self.nurseModel.D - 3:
                        allowFree = True
                    else:
                        allowFree = sum(self.currentSol.solution[i][d-2]) == 0 or sum(self.currentSol.solution[i][d-1]) == 0 or sum(self.currentSol.solution[i][d+1]) == 0 or sum(self.currentSol.solution[i][d+2]) == 0
                else:
                    if d < 2 or d > self.nurseModel.D - 3:
                        allowFree = True
                    else:
                        allowFree = sum(self.currentSol.solution[i][d-2]) == 1 or sum(self.currentSol.solution[i][d-1]) == 1 or sum(self.currentSol.solution[i][d+1]) == 1 or sum(self.currentSol.solution[i][d+2]) == 1
    ...
    #self.nurseModel.model.m.setParam("BestObjStop", 0.5*self.penalties.best)
